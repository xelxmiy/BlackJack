...
public class BlackJack {
    //i'm definitly losing points for the incosistant variable capitalisisation
    //init variables
    ...
    public static void StartBJLoop() {
        ...
        while(Turn.equals("player")  && PlayerTotal < 22){
            ...
            for(int i = 0; i < PlayersCards.size(); i++) {
                if(i != PlayersCards.size() - 1) myCards += PlayersCards.get(i) + ", ";  
                else myCards += "and a " + PlayersCards.get(i);
            }
            ...
            while(Turn.equals("dealer")  && DealerTotal < 22){
               ...
               if(DealersHits()) {
                   Hit("Dealer");
                   if(DealerTotal > 21) GameEnd("dealer", true);
               }
               else {     
                    dealerCards = "";
                    for(int i = 0; i < DealersCards.size(); i++) {
                        if(i != DealersCards.size() - 1) myCards += DealersCards.get(i) + ", ";  
                        else dealerCards += "and a " + DealersCards.get(i);          
                    }
    ...
    //dealer's choise logic, i'm pretty bad at blackjack if you couldn't tell so i'm mkaing shit up
    public static boolean DealersHits() {
        if(DealerTotal >= 16) return false;
        if(DealerTotal == 15 && PlayersCards.get(0) > 7) return true;      
        if(PlayersCards.get(0) > 10) return true;
        if(PlayersCards.get(0) < 8 && PlayersCards.get(0) > 4) return true;
        if(DealerTotal <= 14) return true;
        return false;
    }
